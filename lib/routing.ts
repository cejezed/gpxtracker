...
function osrmBaseUrl() {
  return (process.env.NEXT_PUBLIC_OSRM_BASE_URL ?? DEFAULT_OSRM_BASE_URL).replace(/\/+$/, "");
}

function readRoutePoints(value: unknown): RoutePoint[] {
...
export async function buildRoadRoute(points: RoutePoint[]): Promise<RoadRouteResult> {
  ...
  const response = await fetch(
    `${osrmBaseUrl()}/route/v1/driving/${coordinates}?overview=full&geometries=geojson&steps=false&alternatives=false&generate_hints=false`,
    {
      headers: {
  ...
  const routePoints = readRoutePoints(route?.geometry?.coordinates);
